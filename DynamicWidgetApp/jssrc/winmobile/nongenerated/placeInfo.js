function hBoxforPlaceInfoOuter() {
    var hboxbasicConf1 = {
        id: "hBoxforPlaceInfoOuterId",
        isVisible: true,
        orientation: constants.BOX_LAYOUT_HORIZONTAL,
        skin: "hBoxTransparentUnrounded"
    };
    var hboxlayoutConf1;
...
function hBoxforPlaceInfo() {
    random = random + 1;
    var hboxbasicConf1 = {
        id: "hBoxforPlaceInfo" + random,
        isVisible: true,
        orientation: constants.BOX_LAYOUT_HORIZONTAL,
        skin: "HboxRoundedSkinJS"
    };
    var hboxlayoutConf1 = {
        containerWeight: 100,
        margin: [2, 2, 2, 2],
...
function vBoxforPlaceInfo() {
    random = random + 1;
    var vboxbasicConf1 = {
        id: "vBoxforPlaceInfo" + random,
        isVisible: true,
        orientation: constants.BOX_LAYOUT_VERTICAL,
        skin: "hboxtransparentRounded"
    };
    var vboxlayoutConf1 = {
        containerWeight: 100,
        margin: [0, 0, 0, 0],
    ...
    for (var i = 1; i <= count; i++) {
        if ((frmDynamicJS["nameId" + i]["text"] != textValue) && (frmDynamicJS["listId" + i]["selectedKey"] != "key0")) {
            ...
            //kony.print(myString)
            if (i == 1) topVbox.add(hBoxForName(textName), hBoxForCountry(textCountry));
            else topVbox.add(lineSeperator(), hBoxForName(textName), hBoxForCountry(textCountry)); //
        }
    }
    return topVbox;
}
...
function createDynamicForm2() {
    ...
    if (deviceInfo.name.indexOf("iPhone") > 0 || deviceInfo.name == "Blackberry") textValue = null;
    else textValue = "";
    try {
        for (var i = 1; i <= count; i++) {
            if ((frmDynamicJS["nameId" + i]["text"] != textValue) && (frmDynamicJS["listId" + i]["selectedKey"] != "key0")) IschkAllList = true;
            else IschkAllList = false;
        }
        if (IschkAllList) {
            var frmLogBasiConf = {
                id: "frmDynamicJS2",
                addWidgets: addWidgetsToDynamicFormTwo,
                skin: "frmSampleSkin",
                headers: [hBoxForHeader2()]
            };
            var frmLayoutConf = {
                percent: true
            };
